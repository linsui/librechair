...
class MemoryUsageChipProvider(val context: Context) : ChipProvider() {
    override fun getItems(context: Context): List<Item> = listOf(Item().apply {
        ...
        title =
                "$availableMegs MB / " +
                        "${if (totalMegs < 0x400) totalMegs else (totalMegs.toDouble() / 0x400).roundToLong()} " +
                        "${if (totalMegs < 0x400) "MB" else "GB"} " +
                        "(${((totalMegs - availableMegs).toDouble() / totalMegs).roundToInt() * 100}%)"
    })
}