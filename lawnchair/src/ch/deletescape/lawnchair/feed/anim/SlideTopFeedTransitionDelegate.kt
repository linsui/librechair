...
class SlideTopFeedTransitionDelegate : AnimationDelegate {
    override fun animate(content: View, background: View, width: Float, progress: Float) {
        ...
        if (background is ViewGroup && background.rtl) {
            background.translationY = (-1 + progress) * background.height * -1
            content.translationX = (-1 + progress) * width * -1
        } else {
            background.translationY = (-1 + progress) * background.height
