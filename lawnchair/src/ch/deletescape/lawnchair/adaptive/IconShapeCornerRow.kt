...
class IconShapeCornerRow(context: Context, attrs: AttributeSet?) : LinearLayout(context, attrs),
    ...
    class CornerShapeAdapter(context: Context) :
            SimpleAdapter(context, getEntries(context), android.R.layout.simple_list_item_1,
                          arrayOf("text"), intArrayOf(android.R.id.text1)) {
        companion object {
            private fun getEntries(context: Context): List<Map<String, String>> {
                return listOf(mapOf("value" to "arc",
                                    "text" to context.getString(R.string.icon_shape_corner_round)),
                              mapOf("value" to "squircle", "text" to context.getString(
                                      R.string.icon_shape_corner_squircle)), mapOf("value" to "cut",
                                                                                   "text" to context.getString(
                                                                                           R.string.icon_shape_corner_cut)))
            }
        }
    }
}
