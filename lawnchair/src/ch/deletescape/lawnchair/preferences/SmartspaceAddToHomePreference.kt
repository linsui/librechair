...
class SmartspaceAddToHomePreference(context: Context, attrs: AttributeSet?) :
    ...
    override fun onResume() {
        isVisible = LauncherAppState.getInstance(context).model.loadedWidgets?.any {
            it.isCustomWidget
        } != true
    }
    ...
    override fun onClick() {
        val homeIntent = OpenWidgetsInitListener().addToIntent(
                Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_HOME).setPackage(
                                BuildConfig.APPLICATION_ID).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
