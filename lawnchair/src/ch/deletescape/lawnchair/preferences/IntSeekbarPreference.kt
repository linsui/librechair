...
open class IntSeekbarPreference @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    override fun updateSummary() {
        mValueText!!.text = String.format(format!!, current.roundToInt() * multiplier)
    }

    override fun persistFloat(value: Float): Boolean {
        return super.persistInt(value.roundToInt())
    }

    override fun getPersistedFloat(defaultReturnValue: Float): Float {
        return super.getPersistedInt(defaultReturnValue.roundToInt()).toFloat()
    }
}
