...
@Keep @Suppress("DEPRECATION") class OWMWeatherDataProvider(
    controller: LawnchairSmartspaceController) :
        ...
        LawnchairPreferences.OnPreferenceChangeListener, CurrentWeatherCallback {

    private val context = controller.context
    private val prefs = Utilities.getLawnchairPrefs(context)
    private val owm by lazy { OpenWeatherMapHelper(prefs.weatherApiKey) }
    private val iconProvider by lazy {
        WeatherIconProvider(context)
    }
    ...
    init {
        prefs.addOnPreferenceChangeListener(this, "pref_weatherApiKey", "pref_weather_city",
                                            "pref_weather_units")
    }
    ...
    override fun onFailure(throwable: Throwable?) {
        ...
        if (prefs.weatherApiKey == context.getString(
                    R.string.default_owm_key) || throwable?.message == apiKeyError) {
    ...
    companion object {
        ...
        private const val apiKeyError =
                "UnAuthorized. Please set a valid OpenWeatherMap API KEY" + " by using the setApiKey method."
    }
}
