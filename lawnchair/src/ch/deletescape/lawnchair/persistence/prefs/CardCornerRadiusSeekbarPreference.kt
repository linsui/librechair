...
class CardCornerRadiusSeekbarPreference(context: Context, attrs: AttributeSet?) :
    ...
    override fun persistFloat(value: Float): Boolean {
        ...
        if ((Launcher.getInstance() as? LawnchairLauncher)?.overlay != null) {
            (Launcher.getInstance() as LawnchairLauncher).overlay?.cardCornerRadius
                    ?.set(value.applyAsDip(context))
        }
        return true
    }
