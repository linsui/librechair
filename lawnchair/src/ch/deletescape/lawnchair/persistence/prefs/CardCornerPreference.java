...
public class CardCornerPreference extends ListPreference {
    public CardCornerPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        setDefaultValue(CardStyleRegistry.ROUND);
        setEntryValues(CardStyleRegistry.ALL.keySet().toArray(new String[0]));
        setEntries(CardStyleRegistry.ALL.keySet()
                .stream()
                .map(ChipStyleRegistry.NAMES::get)
