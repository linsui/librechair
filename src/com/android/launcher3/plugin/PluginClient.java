...
public abstract class PluginClient<T extends IInterface> {
    ...
    /**
     * Returns the interface for the plugin type that this client supports. Should be implemented by
     * extending classes using a static constant field.
     *
    ...
    /**
     * Called when a new plugin has been bound. Extend to setup newly connected plugin.
     *
    ...
    /**
     * Called when a previously connected plugin has been unbound. Extend to handle this case.
     *
    ...
    /**
     * Request a list of a type from all connected plugins, then aggregate the lists into one
     * collection.
     *
    ...
    /**
     * Plugin client that only handles one plugin connection at a time. Supports an additional call
     * that returns a non-list value from the connected plugin.
     */
