...
import ch.deletescape.lawnchair.feed.impl.FeedState
...
class GoSlidingView(context: Context, attrs: AttributeSet) : FrameLayout(context, attrs),
    ...
    private fun setProgress(progress: Float, notify: Boolean) {
        mProgress = progress
        if (notify) {
            mLauncherFeed!!.cbScroll(progress, mDetector.isDraggingOrSettling)
        }
    ...
    protected fun onSwipeInteractionCompleted(targetState: FeedState?) {
        ...
        mLauncherFeed!!.cbScroll(mProgress, mDetector.isDraggingOrSettling)
    }
    ...
    companion object {
        ...
        val PROGRESS: Property<GoSlidingView, Float> =
                object : Property<GoSlidingView, Float>(Float::class.java, "progress") {
                    override fun get(`object`: GoSlidingView): Float? {
                        return `object`.mProgress
                    }

                    override fun set(`object`: GoSlidingView, value: Float?) {
