 ...
 * many common mistakes that human (and computer) HTML document authors make. This tag balancer
 * can:
 * <ul>
...
public class HTMLTagBalancer
     ...
     * Name of the property holding the stack of elements in which context a document fragment
     * should be parsed.
     **/
     ...
     * <em>until</em> the end &lt;/HTML&gt; tag is seen (or synthesized). This is used to ensure
     * that extraneous events after the end of the document element do not make the document stream
     * ill-formed.
     */
    ...
    public void startDocument(XMLLocator locator, String encoding,
        ...
        if (fDocumentHandler != null) {
            XercesBridge.getInstance()
                    .XMLDocumentHandler_startDocument(fDocumentHandler, locator, encoding,
                            nscontext, augs);
        }
    ...
    public void startElement(final QName elem, XMLAttributes attrs, final Augmentations augs)
        ...
        if (isForcedCreation && (elementCode == HTMLElements.TABLE
                || elementCode == HTMLElements.SELECT)) {
        ...
        if (element.parent != null) {
            if (!fSeenRootElement && !fDocumentFragment) {
                ...
                if (preferedParent.code != HTMLElements.HEAD || (!fSeenBodyElement
                        && !fDocumentFragment)) {
                    ...
                    if (depth == -1) { // no parent found
                        ...
                        final boolean parentCreated = forceStartElement(qname, null,
                                synthesizedAugs());
                        if (!parentCreated) {
                            if (!isForcedCreation) {
                                notifyDiscardedStartElement(elem, attrs, augs);
     ...
     * forced)
     */
    private boolean forceStartElement(final QName elem, XMLAttributes attrs,
    ...
    public void endElement(final QName element, final Augmentations augs) throws XNIException {
        ...
        if (depth > 1 && elem.isInline()) {
            ...
            for (int i = 0; i < depth - 1; i++) {
                ...
                if (pelem.isInline()
                        || pelem.code == HTMLElements.FONT) { // TODO: investigate if only FONT
